...
Locate bad UF addresses in REDCap replace them if possible, erase them
if not, then disable accounts with no primary email address.
...
message body to locate bad email addresses. It searches UF person data
for a suitable replacement email address. If it finds one it replaces
the bad address where it occurs. Otherwise it erases the bad email
address from REDCap.
